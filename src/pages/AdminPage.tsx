...
export const AdminPage: React.FC = () => {
  ...
  const fetchClients = useCallback(async () => {
    try {
      ...
      const records: ClientRecord[] = snapshot.docs.map((doc) => {
        const d = doc.data();
        const personalData = (d.personalData || d.pd || d.data) as
  ...
  const stats = {
    ...
    thisWeek: clients.filter((c) => Date.now() - c.createdAt.getTime() < 7 * 24 * 60 * 60 * 1000)
      .length,
  };
    ...
    <div className="space-y-6">
      ...
      <Card variant="default" padding="md">
        <CardContent>
          <Input
            placeholder={
              activeTab === 'clients' ? 'Search by name or email...' : 'Search messages...'
            }
            value={searchTerm}
      ...
      {activeTab === 'messages' && (
        <div className="space-y-4">
          {loading ? (
            ...
            <Card variant="default" padding="lg">
              <p className="text-center text-slate-500 dark:text-slate-400">No messages found.</p>
            </Card>
          ) : (
